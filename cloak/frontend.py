...
def compile_cloak_file(input_file_path: str, output_dir: str, import_keys: bool = False, **kwargs):
    """
    ...
    :param import_keys: | if false, zk-snark of all modified circuits will be generated during compilation
                        ...
                        | This option is mostly used internally when connecting to a cloak contract provided by a 3rd-party
    :raise CloakCompilerError: if any compilation stage fails
    :raise RuntimeError: if import_keys is True and cloak file, manifest file or any of the key files is missing
    """
...
def extract_zkay_package(zkp_filename: str, output_dir: str):
    ...
    try:
        ...
        with Manifest.with_manifest_config(manifest):
            compile_cloak_file(cloak_filename, output_dir, import_keys=True)
