...
class Versions:
    CLOAK_SOLC_VERSION_COMPATIBILITY = NpmSpec('^0.6.0')
    ...
    def set_solc_version(version: str):
        ...
        if version == 'latest':
            ...
            try:
                v = Version(version)
                if not Versions.CLOAK_SOLC_VERSION_COMPATIBILITY.match(v):
                    raise ValueError(f'Zkay only supports solc versions satisfying {Versions.CLOAK_SOLC_VERSION_COMPATIBILITY.expression}')
