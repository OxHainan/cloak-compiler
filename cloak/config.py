...
class Config(UserConfig):
    ...
    def get_zk_internal_name(self, fct) -> str:
        if fct.requires_verification_when_external:
            fct_name = fct.name[0].upper() + (fct.name[1:] if len(fct.name)>1 else '')
            return f'_{self.zk_reserved_name_prefix}{fct_name}'
        else:
            return fct.name
    ...
    def get_tee_internal_name(self, fct) -> str:
        if fct.requires_verification_when_external:
            fct_name = fct.name[0].upper() + (fct.name[1:] if len(fct.name)>1 else '')
            return f'_{self.tee_reserved_name_prefix}{fct_name}'
        else:
            return fct.name
    ...
    def zk_prover_key_hash_name(self) -> str:
        return f'{self.zk_reserved_name_prefix}ProverKeyHash'
    ...
    def tee_func_hash_name(self) -> str:
        return f'{self.tee_reserved_name_prefix}FHash'

    @property
    def tee_old_state_hash_name(self) -> str:
        return f'{self.tee_reserved_name_prefix}OSHash'

    @property
    def tee_old_state_name(self) -> str:
        return f'{self.tee_reserved_name_prefix}OS'
    ...
    def tee_field_prime_var_name(self) -> str:
        return f'{self.tee_reserved_name_prefix}FieldPrime'

    @property
    def tee_prover_key_hash_name(self) -> str:
        return f'{self.tee_reserved_name_prefix}ProverKeyHash'
