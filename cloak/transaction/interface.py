...
from cloak.utils.progress_printer import success_print
...
class CloakBlockchainInterface(metaclass=ABCMeta):
    """
    ...
    For safety reasons, Cloak always verifies the integrity of remote contracts by comparing the evm bytecode \
    from the blockchain with the output obtained via local compilation of the corresponding source files.

    (Cloak ensures reproducibility via hard-coded solc versions/settings for global library contracts and by \
    ...
    def service_contract(self):
        if self._service_contract is None:
            self._connect_libraries()
    ...
    def set_tee_address(self, sender: AddressValue) -> Any:
        """
        Set TEE address to the cloak service
        
        :param sender: the account generated by cloak runtime in TEE devices 
        """
        assert isinstance(sender, AddressValue)
        zk_print(f"Set TEE address as {sender}")
        return self._set_tee_address(sender.val)

    def get_tee_address(self) -> Any:
        """
        Get TEE address from the cloak service
        
        :param sender: the requester's address who want to request TEE addresss from cloak devices
        """
        return self._get_tee_address()
    ...
    def connect(self, project_dir: str, contract: str, contract_address: AddressValue, user_address: AddressValue) -> Any:
        ...
        * the library contract referenced in the verification contracts matches the correct zkay library contract
        ...
        zk_file = os.path.join(project_dir, 'contract.cloak')
        if not os.path.exists(zk_file):
            raise IntegrityError('No zkay contract found in specified directory')
        verifier_names = []
        if not os.path.exists(os.path.join(project_dir, 'public_contract.sol')):
        ...
        with success_print():
            zk_print(f'OK: Bytecode on blockchain matches local cloak contract')
...
class ZkayKeystoreInterface(metaclass=ABCMeta):
    ...
    def __init__(self, conn: CloakBlockchainInterface):
        self.conn = conn
