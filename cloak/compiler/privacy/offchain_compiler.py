...
def api(name: str, invoker: str = 'self') -> str:
    from cloak.transaction.offchain import ApiWrapper
...
class PythonOffchainVisitor(PythonCodeVisitor):
    ...
    * connect: Get a handle for an already deployed contract (by specifying the on-chain address of the contract). This method automatically verifies the integrity of the remote contract.
    ...
    def visitSourceUnit(self, ast: SourceUnit):
        ...
        from cloak.transaction.types import CipherValue, AddressValue, RandomnessValue, PublicKeyValue
        from cloak.transaction.offchain import {SCALAR_FIELD_NAME}, ContractSimulator, RequireException
        from cloak.transaction.int_casts import *
