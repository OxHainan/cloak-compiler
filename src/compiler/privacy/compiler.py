...
from compiler.privacy.tags import tag, helper_tag, param_tag, tee_tag
...
def compile_ast(ast: AST, output_directory: str, output_file: Optional[str], simulate=False):
	print(f"Begin compiling {ast.names}......")
	"""
...
class FunctionHelper:
	...
	def get_zok_arguments(self):
		if self.ast.privacy_type == FunctionPrivacyType.ZKP:
			...
			return self.tee.proof_helper.zok_arguments
		elif self.ast.privacy_type == FunctionPrivacyType.MPC:
			# TODO: add visitor for mpc
			pass
		return None
...
class SolidityVisitor(CodeVisitor):
	...
	def handle_function_definition(self, ast: ConstructorOrFunctionDefinition):
		if ast.privacy_type == FunctionPrivacyType.ZKP or isinstance(ast, ConstructorDefinition):
			return self.zkp_function_definition(ast)
		elif ast.privacy_type == FunctionPrivacyType.TEE:
	...
	def zkp_function_definition(self, ast: ConstructorOrFunctionDefinition):
		with log_context('compile "ZKP" typed function', ast.name):
			...
			for p in ast.parameters:
				self.function_helper.zok.check_proper_encryption(p)   # out(e, alpha) & in(e, alpha)
			body = '\n'.join(self.pre_simple_statement)  # add 'genHelper[0] = sol;'

			# body
			body += self.visit_list(ast.body.statements)
			zok_code = self.function_helper.zok.code()   # generate .zok code (constraints writed in Zokrates DSL)
	...
	def tee_function_definition(self, ast: ConstructorOrFunctionDefinition):
		with log_context('compile "TEE" typed function', ast.name):
			...
			self.pre_simple_statement = []
			for p in ast.parameters:
				self.function_helper.tee.check_proper_encryption(p)   # out(e, alpha) & in(e, alpha)
			body = '\n'.join(self.pre_simple_statement) + body # add 'genHelper[0] = sol;'

			# body
			zok_code = self.function_helper.tee.code()   # generate .zok code (constraints writed in Zokrates DSL)
			...
			# record number of switches between zokrates and solidity
			my_logging.data('nCrosses', len(self.function_helper.tee.proof_helper.proof_arguments))
		...
		if self.get_function_privacy_type(ast) == FunctionPrivacyType.TEE:
			if isinstance(ast.lhs, IdentifierExpr):
				...
				if isinstance(target, StateVariableDeclaration):
					...
					r = self.function_helper.tee.from_tee(ast.rhs)
					if ast.annotated_type.type_name == TypeName.bool_type() and ast.annotated_type.privacy_annotation.is_all_expr():
						return f'{lhs} = {r} == 1;'
					else:
						return f'{lhs} = {r};'
...
class TeeVisitor(CodeVisitor):
	...
	def from_tee(self, ast: Expression):
		with WantBool(self, False):
			expr = self.visit(ast) # link and delete related function parameters
		...
		func.parameters.append(Parameter([], AnnotatedTypeName(TypeName.uint_type(), ast.annotated_type.privacy_annotation), Identifier(f'{tee_parameter}')))

		# return expression in solidity
		return tee_parameter
	...
	def _from_solidity(self, ast: Expression):
		...
		# collect code logic for computation in TEE
	...
	def visitFunctionCallExpr(self, ast: FunctionCallExpr):
		# TODO: return param list to judge which params is related
		if isinstance(ast.func, BuiltinFunction):
			if not ast.func.is_private:
				return self.from_solidity(ast)
			elif ast.func.op == 'ite':
