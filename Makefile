...
# run unit tests
unit-test:
	./build-docker.sh make -C src test

# test compiling & running an example contract
example-contract:
	# compile example contract
	./build-docker.sh python3 "./src/main.py" --output ./eval-ccs2019/examples/exam/compiled ./eval-ccs2019/examples/exam/exam.sol
	# generate scenario for example contract
	./build-docker.sh ./eval-ccs2019/generate-scenario.sh ./examples/exam
	# run example scenario
	./build-docker.sh ./eval-ccs2019/examples/exam/scenario/runner.sh
